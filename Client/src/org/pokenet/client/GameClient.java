...
public class GameClient extends BasicGame {
	...
	private Font m_fontLarge, m_fontSmall, m_trueTypeFont, m_pokedexfontsmall,
	m_pokedexfontmedium, m_pokedexfontlarge, m_pokedexfontmini,
	m_pokedexfontbetweenminiandsmall;
	...
	public static void main(String[] args) {
		...
		try {
			gameContainer = new AppGameContainer(GameClient.getInstance(), 800,
					600, fullscreen);
		...
		if (started)
			...
			&& !root.getPlayerPopupDialog().isVisible())
				if (m_ourPlayer != null && !m_isNewMap && m_ourPlayer.canMove())
					if (!m_mapMatrix.getCurrentMap().isColliding(m_ourPlayer,
							Direction.Down))
						move(Direction.Down);
...
//			}
		...
		if (m_ui.getNPCSpeech() == null && !m_ui.getChat().isActive()
			...
			&& !BattleManager.getInstance().isBattling()
			&& m_ourPlayer.canMove())
		...
		if (key == KeyManager.getKey(Action.POKEMOVE_1)
				&& BattleManager.getInstance().isBattling()
				&& !getDisplay().containsChild(
						MoveLearningManager.getInstance().getMoveLearning()))
			BattleManager.getInstance().getBattleWindow().useMove(0);
		if (key == KeyManager.getKey(Action.POKEMOVE_2)
				&& BattleManager.getInstance().isBattling()
				&& !getDisplay().containsChild(
						MoveLearningManager.getInstance().getMoveLearning()))
			BattleManager.getInstance().getBattleWindow().useMove(1);
		if (key == KeyManager.getKey(Action.POKEMOVE_3)
				&& BattleManager.getInstance().isBattling()
				&& !getDisplay().containsChild(
						MoveLearningManager.getInstance().getMoveLearning()))
			BattleManager.getInstance().getBattleWindow().useMove(2);
		if (key == KeyManager.getKey(Action.POKEMOVE_4)
				&& BattleManager.getInstance().isBattling()
				&& !getDisplay().containsChild(
						MoveLearningManager.getInstance().getMoveLearning()))
		...
		if (key == KeyManager.getKey(Action.INTERACTION)
			...
			if (m_ui.getNPCSpeech() == null
				...
				ClientMessage message = new ClientMessage(
						ServerPacket.TALKING_START);
				m_session.send(message);
			}
			if (BattleManager.getInstance().isBattling()
					&& getDisplay().containsChild(
							BattleManager.getInstance().getTimeLine()
							.getBattleSpeech())
							&& !getDisplay()
							.containsChild(
									MoveLearningManager.getInstance()
									.getMoveLearning()))
				BattleManager.getInstance().getTimeLine().getBattleSpeech()
				.advance();
			else
				try {
	...
	public void init(GameContainer gc) throws SlickException {
		...
		m_loadImage = m_loadImage
				.getScaledCopy(800.0f / m_loadImage.getWidth());
		...
		/* Setup variables */
		m_fontLarge = new AngelCodeFont(m_filepath + "res/fonts/dp.fnt",
				m_filepath + "res/fonts/dp.png");
		m_fontSmall = new AngelCodeFont(m_filepath + "res/fonts/dp-small.fnt",
				m_filepath + "res/fonts/dp-small.png");
		m_pokedexfontsmall = new AngelCodeFont(m_filepath
				+ "res/fonts/dex-small.fnt", m_filepath
				+ "res/fonts/dex-small.png");
		m_pokedexfontmedium = new AngelCodeFont(m_filepath
				+ "res/fonts/dex-medium.fnt", m_filepath
				+ "res/fonts/dex-medium.png");
		m_pokedexfontlarge = new AngelCodeFont(m_filepath
				+ "res/fonts/dex-large.fnt", m_filepath
				+ "res/fonts/dex-large.png");
		m_pokedexfontmini = new AngelCodeFont(m_filepath
				+ "res/fonts/dex-mini.fnt", m_filepath
				+ "res/fonts/dex-mini.png");
		m_pokedexfontbetweenminiandsmall = new AngelCodeFont(m_filepath
				+ "res/fonts/dex-betweenminiandsmall.fnt", m_filepath
				+ "res/fonts/dex-betweenminiandsmall.png");
		...
		try {
			/* DOES NOT WORK YET!!! */
			m_trueTypeFont = new TrueTypeFont(java.awt.Font.createFont(
					java.awt.Font.TRUETYPE_FONT,
					new File(m_filepath + "res/fonts/PokeFont.ttf"))
		...
		try {
			...
			theme = ThemeManager.createThemeManager(f.getAbsoluteFile().toURI()
					.toURL(), lwjglRenderer);
			root = new GUIPane();
	...
	private void loadSprites() {
		try {
			 ...
			 */
			for (int i = -7; i < 385; i++) {
				final String location = respath + "res/characters/" + i
						+ ".png";
	...
	public void mousePressed(int button, int x, int y) {
		...
		if (button == 1)
			...
			for (Player p : m_mapMatrix.getPlayers())
				if (x >= p.getX() + m_mapMatrix.getCurrentMap().getXOffset()
				&& x <= p.getX() + 32
				+ m_mapMatrix.getCurrentMap().getXOffset()
				&& y >= p.getY()
				+ m_mapMatrix.getCurrentMap().getYOffset()
				&& y <= p.getY() + 40
				+ m_mapMatrix.getCurrentMap().getYOffset())
	...
	public void render(GameContainer gc, Graphics g) throws SlickException {
		...
		if (!m_started) {
			if (m_nextResource != null) {
				...
				g.drawImage(m_loadBarMiddle, 11 + m_loadBarLeft.getWidth(),
						gc.getHeight() - 120, bar * (maxWidth - 13),
						gc.getHeight() - 120 + m_loadBarMiddle.getHeight(), 0,
						0, m_loadBarMiddle.getWidth(),
						m_loadBarMiddle.getHeight());
				g.drawImage(m_loadBarRight, bar * (maxWidth - 13),
						gc.getHeight() - 120);
				g.drawString("Loading,  please wait ... ", 10,
						gc.getHeight() - 90);
				if (LoadingList.get().getRemainingResources() < 1)
					m_started = true;
			}

			// g.drawString("Loading: " + m_nextResource.getDescription(), 10,
			// gc.getHeight() - 90);
			...
			if (!m_isNewMap && m_ourPlayer != null) {
				...
				for (int x = 0; x <= 2; x++)
					for (int y = 0; y <= 2; y++) {
						...
						if (thisMap != null && thisMap.isRendering())
							thisMap.render(
									thisMap.getXOffset() / 2,
									thisMap.getYOffset() / 2,
									0,
									0,
									(gc.getScreenWidth() - thisMap.getXOffset()) / 32,
									(gc.getScreenHeight() - thisMap
											.getYOffset()) / 32, false);
					}
				g.resetTransform();
				try {
				...
				if (m_mapX > -30) {
					...
					if (m_weather.isEnabled()
							&& m_weather.getParticleSystem() != null)
						try {
	...
	public void update(GameContainer gc, int delta) throws SlickException {
		...
		if (!m_started) {
			if (m_ui == null) {
				...
				m_ui.setAllVisible(false);
				System.out
				.println("Loading the files took "
						+ (System.currentTimeMillis() - startTime)
						+ " ms (time from start until you get the language select screen)");
			}
		} else {
			...
			if (lastPressedKey > -2) {
				...
				if (lastPressedKey != KeyManager.getKey(Action.WALK_UP)
						&& lastPressedKey != KeyManager
						.getKey(Action.WALK_LEFT)
						&& lastPressedKey != KeyManager
						.getKey(Action.WALK_DOWN)
						&& lastPressedKey != KeyManager
						.getKey(Action.WALK_RIGHT))
					lastPressedKey = -2;
			}
			...
			if (m_isNewMap && m_loading.isVisible() && m_ourPlayer != null) {
				...
				m_mapMatrix.getCurrentMap().setName(
						m_mapMatrix.getMapName(m_mapX, m_mapY));
				m_mapMatrix.getCurrentMap().setXOffset(
						400 - m_ourPlayer.getX(), false);
				m_mapMatrix.getCurrentMap().setYOffset(
						300 - m_ourPlayer.getY(), false);
