		...
		switch(message.charAt(0))
		{
			...
			case 'T':
				...
				switch(message.charAt(1))
				{
					...
					case 'o':
						...
						GameClient.getInstance().getUi().getTrade().getOffer(Integer.parseInt(offerData[0]), Integer.parseInt(offerData[1]));
						break;
					case 'c':
					...
					case 'i':
						...
						GameClient.getInstance().getUi().getTrade().addPoke(Integer.parseInt(String.valueOf(message.charAt(2))), data);
						break;
					case 'f':
						// Trading is done
			...
			case 'S':
				...
				switch(message.charAt(1))
				{
					...
					case 'd': // You don't have the item you're trying to sell!
						GameClient.messageDialog("You don't have a " + message.substring(2), GameClient.getInstance().getDisplay());
						break;
					case 'b': // Bought Item
						try
						{
						...
						GameClient.getInstance().getUi().talkToNPC("You bought a " + ItemDatabase.getInstance().getItem(Integer.parseInt(message.substring(2))).getName());
					...
					case 's': // Sold Item
						...
						GameClient.getInstance().getUi().talkToNPC("You sold a " + ItemDatabase.getInstance().getItem(Integer.parseInt(message.substring(2))).getName());
						GameClient.getInstance().getUi().getShop().m_timer.reset();
						GameClient.getInstance().getUi().getShop().m_timer.resume();
						break;
			...
			case 'b':
				...
				switch(message.charAt(1))
				{
					case 'i':
						...
						BattleManager.getInstance().startBattle(message.charAt(2), Integer.parseInt(message.substring(3)));
						break;
					case 'I':
					...
					case 'P':
						...
						BattleManager.getInstance().setEnemyPoke(Integer.parseInt(data[0]), data[1], Integer.parseInt(data[2]), Integer.parseInt(data[3]), Integer.parseInt(data[4]),
								Integer.parseInt(data[5]), Integer.parseInt(data[6]), Boolean.valueOf(data[7]));
						break;
					case 'n':
					...
					case 'F':
						...
						BattleManager.getInstance().getTimeLine().informFaintedPoke(message.substring(2));
						break;
					case 'M':
						// A move was used -> bMPOKEMON,MOVENAME
						BattleManager.getInstance().getTimeLine().informMoveUsed(message.substring(2).split(","));
						break;
					case 'm':
						// Move requested
					...
					case '.':
						...
						BattleManager.getInstance().getTimeLine().informExperienceGained(message.substring(2).split(","));
						break;
					case 'e':
						// A Pokemon received a status effect -> beTRAINER.POKEMON,EFFECT
						BattleManager.getInstance().getTimeLine().informStatusChanged(Integer.parseInt(String.valueOf(message.charAt(2))), message.substring(3).split(","));
						break;
					case 'E':
						// A Pokemon had a status effect removed -> bEPOKEMON,EFFECT
						BattleManager.getInstance().getTimeLine().informStatusHealed(Integer.parseInt(String.valueOf(message.charAt(2))), message.substring(3).split(","));
						break;
					case 's':
					...
					case 'S':
						...
						BattleManager.getInstance().getTimeLine().informSwitch(message.substring(2).split(","));
						break;
					case 'h':
						/*
						 * Receiving health info (health value, not health lost) (don't ask why there's two methods for this, shoddy needs em for some reason) NOTE: 0 is always our player
						 */
						switch(message.charAt(2))
						{
							case '0':
								// Our pokemon's health
								BattleManager.getInstance().getTimeLine().informHealthChanged(message.substring(2).split(","), 0);
								break;
							case '1':
								// Enemy pokemon's health
								BattleManager.getInstance().getTimeLine().informHealthChanged(message.substring(2).split(","), 1);
								break;
						}
						break;
					case 'r':
					...
					case '$':
						// Receiving earnings
						BattleManager.getInstance().getTimeLine().informMoneyGain(Integer.parseInt(message.substring(2)));
						break;
					case 'l':
			...
			case 'P':
				...
				switch(message.charAt(1))
				{
					...
					case 'm':
						/*
						 * Move learning by levelling up, received as (for example) Pm0EMBER The 3rd character is the index of the pokemon in the player's party and the rest is the move name NOTE: This packet is for informing the client a move wants to be learned if the client allows the move learning, the server does not send a reply to confirm it was learned, as it is ensured it is learned
						 */
						MoveLearningManager.getInstance().queueMoveLearning(Integer.parseInt(String.valueOf(message.charAt(2))), message.substring(3));
						break;
					case 'M':
						/* Move learning by TM, this packet is confirmation that the moved was learned */
						m_game.getOurPlayer().getPokemon()[Integer.parseInt(String.valueOf(message.charAt(2)))].setMoves(Integer.parseInt(String.valueOf(message.charAt(3))), message.substring(4));
					...
					case 'E':
						/*
						 * A pokemon would like to evolve, received as PE1 1 is the index of the pokemon in the party Reply to server with the same packet except with a 0 at the end if the evolution was cancelled or a 1 if it was allowed, e.g. PE10
						 */
			...
			case 'F':
				...
				switch(message.charAt(1))
				{
					case 'F': // You can't use that rod at your current level
						GameClient.messageDialog("You need to have a fishing level of " + message.substring(2) + " to use that rod!", GameClient.getInstance().getDisplay());
						break;
			...
			case 'c':
				...
				switch(message.charAt(1))
				{
					case 'h':
						...
						GameClient.messageDialog("You are not strong enough to do this.\n" + "Your trainer level must be " + message.substring(2) + " to do this.", GameClient.getInstance()
								.getDisplay());
						break;
					case 'B':
						// Badge change
						switch(message.charAt(2))
						{
							case 'i':
								/*
								 * All of the player's badges. Received as 0000001100001110000 0 = no badge 1 = badge earned The string will be 41 chars long 0 - 7 Kanto Badges 8 - 15 Johto Badges 16 - 23 Hoenn Badges 24 - 31 Sinnoh Badges 32 - 35 Orange Islands Badges 36 - 41 Undefined
								 */
			...
			case 'M':
				...
				{
					processMovement(Integer.parseInt(details[i].substring(1)), details[i].charAt(0));
				}
				break;
			case 'm':
				// Map Information
				switch(message.charAt(1))
				{
					...
					case 'F':
						...
						switch(message.charAt(2))
						{
							case 'i':
								// Initialise
								GameClient.getInstance().getUi().initFriendsList(message.substring(3).split(","));
								break;
							case 'a':
								// A friend was added
								GameClient.getInstance().getUi().getFriendsList().addFriend(message.substring(3));
								break;
							case 'r':
			...
			case 'l':
				...
				switch(message.charAt(1))
				{
					case 's':
						...
						m_game.getTimeService().setTime(Integer.parseInt(details[1].substring(0, 2)), Integer.parseInt(details[1].substring(2)));
						break;
					case 'u':
					...
					case '4':
						// Banned
						GameClient.messageDialog("You've been banned.", GameClient.getInstance().getDisplay());
						m_game.getLoadingScreen().setVisible(false);
						m_game.getLoginScreen().showLogin();
						break;
				}
