...
public class PlayerInfoDialog extends Frame {
	...
	private void initGUI() {
		...
		//if (ALL_REGIONS)
			m_hoennLbl.pack();
			m_hoennLbl.setForeground(Color.white);
		//if (ALL_REGIONS)
			m_sinnohLbl.pack();
			m_sinnohLbl.setForeground(Color.white);
		if (ALL_REGIONS)
			m_orangeLbl.pack();
		if (ALL_REGIONS)
			m_extrasLbl.pack();
		...
		if (ALL_REGIONS) {
			...
			setSize(160 + 2 + maxLblWidth, m_sinnoh[0].getY()
					+ m_sinnoh[0].getHeight() + getTitleBar().getHeight() + 2);
		}
