...
{
	/* TODO: Cleanup this class, redundant data and sloppy code.* */
	...
	private void initGUI()
	{
		...
		// Player Image
		m_playerImage = new Label(Player.getSpriteFactory().getSprite(Direction.Down, false, false, GameClient.getInstance().getOurPlayer().getSprite()));
		...
		m_breedingEXP.setLocation(m_playerImage.getWidth() + 2, 2 + m_trainerEXP.getY() + m_trainerEXP.getHeight());
		m_fishingEXP.setLocation(m_playerImage.getWidth() + 2, 2 + m_breedingEXP.getY() + m_breedingEXP.getHeight());
		m_coordinatingEXP.setLocation(m_playerImage.getWidth() + 2, 2 + m_fishingEXP.getY() + m_fishingEXP.getHeight());
		...
		m_kantoLbl.setY(m_coordinatingEXP.getY() + m_coordinatingEXP.getHeight() + 4);
		...
		/* DEAD CODE WARNING CAN BE IGNORED, THIS FILE WILL NEED TO BE REFACTORED ANYWAYZ ~TMXOD/apoliak */
		maxLblWidth = ((ALL_REGIONS) ? m_orangeLbl : m_kantoLbl).getWidth();
		loadImages();
		/* 6 rows, 20 pixels each + title bar height */
		if(ALL_REGIONS)
			setSize(160 + 2 + maxLblWidth, m_extras[0].getY() + m_extras[0].getHeight() + getTitleBar().getHeight() + 2);
		else
			setSize(160 + 2 + maxLblWidth, m_johto[0].getY() + m_johto[0].getHeight() + getTitleBar().getHeight() + 2);
	...
	public void loadImages()
	{
		...
		for(int i = 0; i < 8; i++)
		{
			...
			{
				// KANTO
				m_kanto[i] = new Label(new Image(m_path + "kanto" + (i + 1) + ".png", false));
				m_kanto[i].setSize(18, 18);
				m_kanto[i].setX(maxLblWidth + (20 * i));
				m_kanto[i].setY(m_coordinatingEXP.getY() + m_coordinatingEXP.getHeight() + 2);
				getContentPane().add(m_kanto[i]);
				// JOHTO
				m_johto[i] = new Label(new Image(m_path + "johto" + (i + 1) + ".png", false));
				...
				if(ALL_REGIONS)// ORANGE ISLANDS
				{
					if(i < 4)
					{
						...
						m_orange[i].setY(2 + m_sinnoh[i].getY() + m_sinnoh[i].getHeight());
						getContentPane().add(m_orange[i]);
					}
				}
				if(ALL_REGIONS)// Extra badges ???
				{
					if(i < 6)
					{
						...
						m_extras[i].setY(2 + m_orange[i].getY() + m_orange[i].getHeight());
	...
	/**
	 * Shows badges (darkens ones the player does not have yet).
	 * 
