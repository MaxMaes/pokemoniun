...
public class LoginManager implements Runnable {
	...
	private void attemptLogin(Session session, char l, String username, String password) {
		try {
			...
			ResultSet result = m_database.query("SELECT * FROM pn_bans WHERE ip='" + session.getIpAddress() + "'");
			...
			if(result.getString("password").compareTo(password) == 0) {
				...
				if(result.getString("lastLoginServer").equalsIgnoreCase(GameServer.getServerName())) {
					...
					if(ActiveConnections.getPlayer(username) != null) {
						PlayerChar p = ActiveConnections.getPlayer(username);
...
//					}
	...
	private void login(String username, char language, Session session, ResultSet result) {
		...
		p.setLastLoginTime(time);
		//p.setSession(session);not sure if this can be removed!!!
		session.setPlayer(p);
	 ...
	 * @param session
	 */
	private void initialiseClient(PlayerChar p, Session session) {
		ServerMessage message = new ServerMessage();
		message.Init(74);
