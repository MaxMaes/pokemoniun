...
import org.pokenet.server.network.NetworkManager;
...
{
	private NetworkManager m_networkService;
	...
	public ServiceManager()
	{
		...
		m_networkService = new NetworkManager();
	...
	{
		/*
		 * Start the network service first as it needs to bind the address/port to the game server. Then start all other services with TimeService last.
		 */
	...
	{
		/*
		 * Stopping services is very delicate and must be done in the following order to avoid leaving player objects in a non-concurrent state.
		 */
		m_timeService.stop();
