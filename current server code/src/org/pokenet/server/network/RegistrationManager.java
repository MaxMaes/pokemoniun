...
public class RegistrationManager implements Runnable {
	...
	public void register(IoSession session) throws Exception {
		...
		ResultSet id = m_database.query("SELECT pokedexid FROM pn_pokedex WHERE memberid = " + MySqlManager.parseSQL(""+playerId));
		id.first();
		//Get the pokedex ID
		int pokedexid = id.getInt("pokedexid");
		//Bind the pokedex ID to the member
		m_database.query("UPDATE pn_members SET pokedexId = " + MySqlManager.parseSQL(""+pokedexid) + " WHERE id = " + MySqlManager.parseSQL(""+playerId));
		//Add the players starter to the pokedex
		m_database.query("UPDATE pn_pokedex SET " + "`" + MySqlManager.parseSQL("" + p.getSpeciesNumber()) + "`" + " = '2' WHERE pokedexid = '" + MySqlManager.parseSQL("" + pokedexid) + "'");
