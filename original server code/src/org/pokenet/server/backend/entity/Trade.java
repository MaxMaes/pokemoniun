...
public class Trade implements Runnable
{
	...
	public Trade(Tradeable player1, Tradeable player2)
	{
		...
		/*
		 * if(player1.getIpAddress().equalsIgnoreCase(player2.getIpAddress())) { if(player1 instanceof Player) { Player p = (Player) player1; p.getTcpSession().write("!Trading cannot be done with that player"); } endTrade(); return; }
		 */
				...
				{ // do both pokemon
					...
					for(PokemonEvolution currentEvolution : pokeData.getEvolutions())
					{
						...
						else if(currentEvolution.getType() == EvolutionTypes.TradeItem)
						{
							...
							else if(curPokemon.getItem().getName().equalsIgnoreCase("UP-GRADE") && currentEvolution.getAttribute().equalsIgnoreCase("UP_GRADE"))
							{
								curPokemon.setEvolution(currentEvolution);
								curPokemon.evolutionResponse(true, p);
							}
