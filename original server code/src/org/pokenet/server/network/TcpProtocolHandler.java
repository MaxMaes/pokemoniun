...
import org.pokenet.server.ServiceManager;
...
public class TcpProtocolHandler extends IoHandlerAdapter {
	...
	public void sessionCreated(IoSession session) {
		Log.debug("Client connected (" + getIp(session) + ")");
	 ...
	 * @return
	 */
	public static String getIp(IoSession s) {
		if (s != null) {
	...
	public void exceptionCaught(IoSession session, Throwable t) {
		...
		if (!(t instanceof IOException) || session.isConnected()
				|| !session.isClosing())
			session.close(true);
		t.printStackTrace();
	}
	 ...
	 * @param Object
	 *            msg - The packet received from the client
	 */
	public void messageReceived(IoSession session, Object msg) throws Exception {
		String message = (String) msg;
		String[] details;
		ServiceManager serviceManager = GameServer.getServiceManager();
		if (session.getAttribute("player") == null) {
			/*
			 * The player hasn't been logged in, only allow login and
			...
			case 'c':
				...
				details = message.substring(1).split(",");
				m_loginManager.queuePasswordChange(session, details[0],
						details[1], details[2]);
				break;
			case 'f':
				// Force login
			...
			case 'P':
				...
				case 'm':
					...
					pokemonIndex = Integer.parseInt(String.valueOf(message
							.charAt(2)));
					int moveIndex = Integer.parseInt(String.valueOf(message
							.charAt(3)));
					move = message.substring(4);
					if (move != null && !move.equalsIgnoreCase("")
							&& p.getParty()[pokemonIndex] != null) {
						if (p.getParty()[pokemonIndex].getMovesLearning()
								.contains(move)) {
							p.getParty()[pokemonIndex].learnMove(moveIndex,
									move);
				...
				case 'M':
					...
					if (p.getParty()[pokemonIndex] != null) {
						if (p.getParty()[pokemonIndex].getMovesLearning()
								.contains(move)) {
							p.getParty()[pokemonIndex].getMovesLearning()
									.remove(move);
						}
					}
					break;
				case 'e':
					...
					if (p.getParty()[pokemonIndex] != null) {
						...
						case '0':
							// Cancel evolution
							p.getParty()[pokemonIndex].evolutionResponse(false,
									p);
							break;
						case '1':
							// Allow evolution
							p.getParty()[pokemonIndex].evolutionResponse(true,
									p);
							break;
						}
					}
			...
			case 's':
				// Party swapping
				p.swapPokemon(Integer.parseInt(message.substring(1,
						message.indexOf(','))), Integer.parseInt(message
			...
			case 'S':
				...
				if (p.isShopping()) {
					...
					case 'b':
						...
						item = Integer.parseInt(message.substring(2,
								message.indexOf(',')));
						// int q =
						// Integer.parseInt(message.substring(message.indexOf(',')
						// + 1));
						p.buyItem(item, 1);
						break;
					case 's':
					...
					if (sprite > 0
							&& !serviceManager.getSpriteList()
									.getUnbuyableSprites().contains(sprite)) {
						if (p.getMoney() >= 500) {
			...
			case 'r':
				...
				case 'l':
					...
					if (m_players.containsKey(player)) {
						...
						writeMessage(
								m_players.get(player).getTcpSession(),
								new RequestMessage(RequestType.RESPONSE, p
										.getName()));
				...
				case 'b':
					...
					if (m_players.containsKey(player)) {
						writeMessage(
								m_players.get(player).getTcpSession(),
								new RequestMessage(RequestType.BATTLE, p
										.getName()));
				...
				case 't':
					...
					if (m_players.containsKey(player)) {
						writeMessage(
								m_players.get(player).getTcpSession(),
								new RequestMessage(RequestType.TRADE, p
										.getName()));
			...
			case 'B':
				...
				if (p.isBoxing()) {
					...
					case 'r':
						...
						int boxNum = Integer.parseInt(String.valueOf(message
								.charAt(2)));
						if (boxNum >= 0 && boxNum < 9)
							p.sendBoxInfo(boxNum);
						break;
					case 'R':
						// Releasing a pokemon from storage - sent as
						// BRBOXNUM,BOXSLOT
						details = message.substring(2).split(",");
						p.releasePokemon(Integer.parseInt(details[0]),
								Integer.parseInt(details[1]));
						break;
					case 's':
						// Swap pokemon between box and party - sent as
						// BsBOXNUM,BOXSLOT,PARTYSLOT, e.g.Bs0,1,0
						details = message.substring(2).split(",");
			...
			case 'M':
				...
				if (message.charAt(1) == 'c') {
					p.getTcpSession().write(
							"Cl" + m_players.size() + " players online");
				} else if (p.getAdminLevel() > 0) {
					switch (message.charAt(1)) {
					case 'a':
						// Server announcement
						for (String s : m_players.keySet()) {
							m_players.get(s).getTcpSession()
									.write("q" + message.substring(2));
						}
						break;
					case 'l':
						// Send an alert
					...
					case 'w':
						...
						case 'n':
							// Normal
							serviceManager.getTimeService().setForcedWeather(0);
							break;
						case 'r':
							// Rain
							serviceManager.getTimeService().setForcedWeather(1);
							break;
						case 's':
							// Snow/Hail
							serviceManager.getTimeService().setForcedWeather(2);
							break;
						case 'f':
							// Fog
							serviceManager.getTimeService().setForcedWeather(3);
							break;
						case 'S':
							// Sandstorm
							serviceManager.getTimeService().setForcedWeather(4);
							break;
						case 'R':
							// Random
							serviceManager.getTimeService().setForcedWeather(9);
							break;
						}
						break;
					case 's':
					...
					case 'n':
						...
						if (p.getAdminLevel() == 2) {
							...
							serviceManager
									.getNetworkService()
									.getChatManager()
									.queueLocalChatMessage("<SERVER> " + mes,
											p.getMapX(), p.getMapY(),
											p.getLanguage());
						}
						break;
			...
			case 'b':
				...
				if (p.isBattling()) {
					...
					case 'm':
						...
						turn = BattleTurn.getMoveTurn(Integer.parseInt(message
								.substring(2)));
					...
					case 's':
						...
						if (p.getParty()[pIndex] != null) {
							if (!p.getParty()[pIndex].isFainted()) {
								...
								p.getBattleField().queueMove(p.getBattleId(),
										turn);
							}
						}
						break;
			...
			case 'i':
				// Drop item
				if (p.getBag().removeItem(
						Integer.parseInt(message.substring(1)), 1)) {
					writeMessage(p.getTcpSession(), new ItemMessage(false,
							Integer.parseInt(message.substring(1)), 1));
				}
				break;
			case 'T':
				...
				if (p.isTrading()) {
					...
					case 'o':
						...
						details = message.substring(2).split(",");
						p.getTrade().setOffer(p,
								Integer.parseInt(String.valueOf(details[0])),
				...
				case 'p':
					...
					if (m_players.containsKey(details[0])) {
						serviceManager
								...
								.queuePrivateMessage(
										details[1],
										m_players.get(details[0])
												.getTcpSession(), p.getName());
					}
					break;
				case 't':
					// Start talking
	...
	public void sessionClosed(IoSession session) {
		...
		if (p != null) {
			if (p.isBattling()) {
				...
				if (p.getBattleField() instanceof PvPBattleField) {
					((PvPBattleField) p.getBattleField()).disconnect(p
							.getBattleId());
				}
		...
		Log.debug("Client disconnected (" + getIp(session) + ")");
	}
	...
	public void logoutAll() {
		...
		/*
		 * Since the method is called during a server shutdown, wait for all
		 * players to be logged out
		 */
