...
public class GameClient extends BasicGame {
	...
	private static String CHATHOST = "67.249.80.136";
	//UI
	...
	private static boolean m_languageChosen = true;
	...
	static {
		try {
			...
			m_instance = new GameClient("Pokemonium 1.0");
	...
	public void render(GameContainer gc, Graphics g) throws SlickException {
		g.drawImage(m_loadImage, 0, 0);
		 ...
		 if (m_nextResource != null) {
			...
			g.drawString("Pokemonium Client is Loading: Please wait", 10, gc.getHeight() - 90);
		}
		...
		if(m_started){
			...
			if (key == (Input.KEY_ESCAPE)) {
				if(m_confirm==null){
					...
					m_confirm = new ConfirmationDialog("Are you sure you want to exit from Pokemonium ?",yes,no);
...
//							"Could not connect to chat server.", getDisplay());
	...
	public static void main(String [] args) {
		...
		try {
			gc = new AppGameContainer(new GameClient("Pokemonium 1.0"),
					800, 600, fullscreen);
